...
void Window::_init_os_window()
{
    ...
    _win32_window = CreateWindowEx(0,
        (LPCWSTR)_win32_class_name.c_str(),		// class name
        (LPCWSTR)_window_name.c_str(),			// app name
        style,							// window style
...
void Window::_deinit_os_window()
{
    DestroyWindow(_win32_window);
    UnregisterClass((LPCWSTR)_win32_class_name.c_str(), _win32_instance);
}

void Window::_update_os_window()
{
    MSG msg;
